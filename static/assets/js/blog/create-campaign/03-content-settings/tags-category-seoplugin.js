...
`;
    ...
    function renderTagsCard(parent) {
      ...
      card.innerHTML = `
<h3><i class="entypo-tag"></i> Tags</h3><span style="color: red;">*</span>
...
`;
      ...
      // Advanced toggle
      $('#tagAdvToggle').off('click').on('click', function () {
        console.log('adv toggle');
        const show = $('#tagAdvBox').is(':visible');
        console.log('adv toggle', show);
...
function renderCategoryCard(parent) {
  $('#metaCategoryCard').remove();
  const card = el('div', { class: 'meta-card', id: 'metaCategoryCard' });
  
  // --- MODIFIED HTML ---
  // We've added a 'category-loading-message' div and changed the select ID
  card.innerHTML = `
    <h3><i class="entypo-folder"></i> Category</h3><span style="color: red;">*</span>
    
    <!-- This div will show "Loading..." or error messages -->
    <div id="category-loading-message" style="color: #666; font-size: 13px; padding: 10px 0;">
      Checking for WordPress connection...
    </div>

    <!-- The select element now has the correct ID and is initially hidden -->
    <select id="category-selector" class="meta-select" style="display: none;">
      <option value="">Select category…</option>
    </select>
    
    <!-- The "New category" button can remain -->
    <span class="advanced-toggle" id="newCatBtn" style="margin-top:10px; display: none;">+ New category</span>
  `;
  parent.appendChild(card);
  ...
  const $sel = $('#category-selector');
      ...
      $('#newCatBtn').off('click').on('click', function () {
...
`;
    ...
    /* ---------- INIT DOM READY ---------- */
    $(function () {
      if (window.__metaPanelInitialized) return;
      window.__metaPanelInitialized = true;
