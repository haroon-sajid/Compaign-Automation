...
`;
  ...
  function togglePanel() {
    ...
    console.log('my blog togglePanel', { manual, coverSel, contSel, cardSel });
  ...
  function setMapValue(type, idx, src) { imgMap[type][idx] = src; storeMap(); }
  function clearMapValue(type, idx) { delete imgMap[type][idx]; storeMap(); }

  function getContentList(idx) {
    const v = imgMap.content[idx];
    if (Array.isArray(v)) return v.slice();
    if (typeof v === 'string' && v) return [v];
    return [];
  }
  function setContentList(idx, arr) {
    if (arr && arr.length) imgMap.content[idx] = arr.slice();
    else delete imgMap.content[idx];
    storeMap();
  }
  ...
  function autoMatch() {
    clearAll(activeType);
    $(SEL.listBoxes).filter(`[data-type="${activeType}"]`).each(function () {
      const kw = norm($(this).closest('.mi-row').find('.title').text());
      const $im = $(SEL.poolImgs).filter(function () { return norm(this.dataset.name || '') === kw; }).first();
      if ($im.length) placeImg($(this), $im.attr('src'));
    });
  }
