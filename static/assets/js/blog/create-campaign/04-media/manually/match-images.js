...
async function autoMatch() {
  ...
  // Check if we're in AI mode - if so, skip the image source validation
  const isAIMode = window.isAIModeActive && window.isAIModeActive();
  if (!imagePlatform && !isAIMode) {
  ...
  const promises = Array.from(keywordRows).map(row => {
    ...
    // // ------------------------------------------------------------------
    // // 1.  Guard: refuse to run if platform is missing
    // // ------------------------------------------------------------------
    // if (!imagePlatform) {
    //   alert('Please choose an image source (Pexels, Unsplash …) before auto-matching.');
    //   return;               // <-- early exit, no 400 ever fired
    // }
    ...
    // // ------------------------------------------------------------------
    // // 3.  Build promises only when platform is known
    // // ------------------------------------------------------------------
    // const promises = Array.from(keywordRows).map(row => {
    //   const keyword  = row.dataset.keyword;
    //   const coverBox = row.querySelector('.thumb-box[data-type="cover"]');
    ...
    //   const baseUrl = window.autoMatchUrl || '/api/media/auto-match/';
    //   const url = `${baseUrl}?keyword=${encodeURIComponent(keyword)}&imageSourcePlatform=${encodeURIComponent(imagePlatform)}`;

    //   return fetch(url)
    //     .then(r => r.ok ? r.json() : null)
    //     .then(data => {
    //       if (data?.status === 'success' && Array.isArray(data.images) && data.images.length) {
    //         // …keep your current image-injection code here…
    //       }
    //     })
    //     .catch(err => {
    //       console.warn(`Auto-match failed for “${keyword}”`, err);
    //       return null;   // swallow so other rows can continue
    //     });
    // });
