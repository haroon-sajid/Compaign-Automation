# campaignhub/management/commands/seedprompts.py

from django.core.management.base import BaseCommand
from accounts.models import Organization
from campaignhub.models import PromptTemplate, Campaign

# This list contains the default prompt templates that we will create.
...
<!-- Reviewer: run 5-minute human sweep for voice, factual accuracy, empathy. -->"""
    },
    {
        ...
        "user_prompt": "Create a 5-7 tweet thread about the topic '{keyword}'. The first tweet should be a strong hook to grab attention. The following tweets should build on the topic with interesting facts or tips. The final tweet should include a call-to-action or a summary. Use 2-3 relevant hashtags in the thread."
    },
    {
        "name": "LinkedIn Post",
        "campaign_type": Campaign.CampaignType.SOCIAL,
...
class Command(BaseCommand):
    help = 'Seeds the database with default prompt templates for all existing organizations.'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS(
            "Starting to seed default prompts..."))
        ...
        for org in organizations:
            ...
            for template_data in DEFAULT_PROMPT_TEMPLATES:
                ...
                if created:
                    self.stdout.write(self.style.SUCCESS(
                        f"    - Created prompt: '{obj.name}'"))
                else:
                    self.stdout.write(
                        f"    - Prompt '{obj.name}' already exists. Skipping.")
